...
public class NFSeContVO {
	...
	public static NfServCont montaNfe(EmissaoVO e) throws Exception{
		...
		for(F55IJC71 it : e.getDetalheNFeList()){
			//if(it.getJCBICM().compareTo(BigDecimal.ZERO) != 0) {
				TpRegImpostoIcms item = TpRegImpostoIcmsVO.montaIcms(it, e.getHeader());
		...
		nfseCont.setCodigoModeloFiscal(e.getHeader().getJCB76MOTY());
		...
		nfseCont.setDmTpAssinante(new BigInteger("1"));		
...
//		if(listTpProcRef != null && !listTpProcRef.isEmpty()) {
