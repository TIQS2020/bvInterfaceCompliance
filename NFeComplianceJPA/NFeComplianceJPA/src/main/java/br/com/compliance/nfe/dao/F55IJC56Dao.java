...
import br.com.compliance.nfe.jde.domain.F55IJC52;
import br.com.compliance.nfe.jde.domain.F55IJC56;
import br.com.compliance.nfe.jde.domain.F55IJC56Id;
import br.com.compliance.nfe.jpa.EntityManagerHelper;

public class F55IJC56Dao {
	
public List<F55IJC56> listF55IJC56ById(F55IJC56Id id) {	
		
		List<F55IJC56> listF55IJC56 = new ArrayList<F55IJC56>();
		
		CriteriaBuilder cb = EntityManagerHelper.getEntityManager().getCriteriaBuilder();
		CriteriaQuery<F55IJC56> cQuery = cb.createQuery(F55IJC56.class);
		Root<F55IJC56> cabecalho = cQuery.from(F55IJC56.class);
		
		/*TODO: [MAPEAMENTO] ASSUMI A CHAVE COMO CRITÉRIO DA BUSCA*/
		//Expression<F55IJC51> eJCCO = cabecalho.get("JCCO");
		Expression<F55IJC52> eJCWJOBS = cabecalho.get("JCWJOBS");
		Expression<F55IJC52> eJCBEGDATE = cabecalho.get("JCBEGDATE");
		Expression<F55IJC52> eJCENDDATE = cabecalho.get("JCENDDATE");
		//Expression<F55IJC51> eJCITM = cabecalho.get("JCITM");
		//Expression<F55IJC51> eJCUOM1 = cabecalho.get("JCUOM1");
		Expression<F55IJC52> eJCEV15 = cabecalho.get("JCEV15");
		...
		*/
		...
		TypedQuery<F55IJC56> query = EntityManagerHelper.getEntityManager().createQuery(cQuery);
		query.setHint(QueryHints.CACHE_STORE_MODE, CacheStoreMode.REFRESH);
						
		if(!query.getResultList().isEmpty()){
			listF55IJC56 = query.getResultList();
		}				
