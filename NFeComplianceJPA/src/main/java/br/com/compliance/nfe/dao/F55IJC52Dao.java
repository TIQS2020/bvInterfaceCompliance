...
import br.com.compliance.nfe.jde.domain.F55IJC52Id;
import br.com.compliance.nfe.jpa.EntityManagerHelper;

public class F55IJC52Dao {
	
	public List<F55IJC52> listF55IJC52ById(F55IJC52Id id) {	
		
		List<F55IJC52> listF55IJC52 = new ArrayList<F55IJC52>();
		
		CriteriaBuilder cb = EntityManagerHelper.getEntityManager().getCriteriaBuilder();
		CriteriaQuery<F55IJC52> cQuery = cb.createQuery(F55IJC52.class);
		Root<F55IJC52> cabecalho = cQuery.from(F55IJC52.class);
		...
		*/
		...
		TypedQuery<F55IJC52> query = EntityManagerHelper.getEntityManager().createQuery(cQuery);
		query.setHint(QueryHints.CACHE_STORE_MODE, CacheStoreMode.REFRESH);
						
		if(!query.getResultList().isEmpty()){
			listF55IJC52 = query.getResultList();
		}				
