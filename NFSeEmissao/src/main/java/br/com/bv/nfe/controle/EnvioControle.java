...
public class EnvioControle {
	...
	public void inicializaProcesso(ServicesVO services, CaminhosVO caminhosVO) {
		...
		try {
			log.info("## INICIANDO PROCESSO DE EMISSAO - "+dateFormat.format(new Date())+" ##");
			...
			if(arquivoVoList.size() > 0) {
				...
				for(ArquivoVo arquivoVo : arquivoVoList) {
					...
					sourceStr = caminhosVO.getRecebido() + "\\" + nomeArquivo;
					destStr = caminhosVO.getProcessando() + "\\" + nomeArquivo;

					if(arquivoVo.isError()){
						sourceStr = caminhosVO.getRecebido() + "\\" + nomeArquivo;
						destStr = caminhosVO.getErro() + "\\" + nomeArquivo;
						xmlFileControl.moveFile(sourceStr, destStr);
						log.error("## ERRO DE INTEGRACAO: ERRO AO INICIALIZAR O PROCESSO COM OS ARQUIVOS E BANCO LOCAL - EXCECAO --> " + arquivoVo.getErrorStr());
					}else{
						if(arquivoVo.getXmlFileEnvioRoot() != null) {
							...
							try {
								...
								sourceStr = caminhosVO.getProcessando() + "\\" + nomeArquivo;
								destStr = caminhosVO.getErro() + "\\" + nomeArquivo;
								xmlFileControl.moveFile(sourceStr, destStr);
	...
	public void envioNFSe(String emissaoServicoURL) throws Exception {
		...
		if (autenticacao != null) {
			...
			log.info("++ Cadastro NFe: Nao ha codigo de autentica��o ++");
		}
	...
	public void cadastraParticipante(String participanteServiceURL) throws Exception {
		...
		if (autenticacao != null) {
			if (envioVO.getParticipanteNFeList() != null) {
				...
				log.info("++ Cadastro Item: N�o h� Participante para serem cadastrados ++");
			}
		} else {
			log.info("++ Cadastro Participante: N�o h� c�digo de autentica��o ++");
		}
	...
	public void cadastraItem(String itemServiceURL) throws Exception {
		...
		if (autenticacao != null) {
			if (envioVO.getDetalheNFeList() != null) {
				...
				log.info("++ Cadastro Item: N�o h� itens para serem cadastrados ++");
			}
		} else {
			log.info("++ Cadastro Item: N�o h� c�digo de autentica��o ++");
		}
	...
	public void cadastrarNaturezaOperacao(String natOpServiceURL) throws Exception {
		...
		log.info("## Cadastro de Natureza Opera��o ##");
		...
		if (autenticacao != null) {
			if (envioVO.getHeader() != null) {
				...
				try {
					...
					LoteIntWSResponse lote = response.getLoteWsResponse();
					log.info(
							"++ Cadastro Natureza Opera��o : Protocolo do lote = " + lote.getNumeroProtocoloLote());
				} catch (ServiceException e) {
				...
				log.info("++ Cadastro Natureza Opera��o: N�o h� Natureza Opera��o para serem cadastradas ++");
			}
		} else {
			log.info("++ Cadastro Natureza Opera��o: N�o h� c�digo de autentica��o ++");
		}
	...
	public void cadastrarUnidade(String unidadeServiceURL) throws Exception {
		...
		if (autenticacao != null) {
			if (envioVO.getDetalheNFeList() != null) {
				...
				log.info("++ Cadastro Unidade: N�o h� Unidades para serem cadastradas ++");
			}
		} else {
			log.info("++ Cadastro Unidade: N�o h� c�digo de autentica��o ++");
		}
