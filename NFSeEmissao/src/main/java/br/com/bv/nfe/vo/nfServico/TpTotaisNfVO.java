...
public class TpTotaisNfVO {
	...
	public static TpTotaisNf montaTotaisNf(F55IJC80 header) throws Exception{
		...
		/*NFE 4.0*/
		totaisNf.setVlIcmsUfDest(BigDecimal.ZERO);
		totaisNf.setVlIcmsUfRemet(BigDecimal.ZERO);
		totaisNf.setVlCombPobrUfDest(BigDecimal.ZERO);
		totaisNf.setVlFCP(header.getJCAG2().divide(new BigDecimal(100)));
		totaisNf.setVlFCPST(BigDecimal.ZERO);
		totaisNf.setVlFCPSTRet(BigDecimal.ZERO);
		totaisNf.setVlIPIDevol(BigDecimal.ZERO);
		
		/*NT 2019.1*/
		totaisNf.setVlIcmsDeson(header.getJCAAMT1()!= null ? header.getJCAAMT1().divide(new BigDecimal("100")) : BigDecimal.ZERO);
